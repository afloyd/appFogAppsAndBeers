...
module.exports = function (app) {
	...
	app.post(/\/auth\/captcha/, function (req, res, next) {
		...
		if (reCaptchaPrivKey) {
			...
			console.error('reCaptcha not configured!! To use reCAPTCHA you must get an API key from https://www.google.com/recaptcha/admin/create');
		...
		Beer.find({}, function (err, beers) {
			...
			if (req.user) {
				...
				if (reCaptchaPubKey) {
					...
					reCaptchaHtml = 'Sorry, reCaptcha not configured. To use reCAPTCHA you must get an API key from https://www.google.com/recaptcha/admin/create';
				}
			}
